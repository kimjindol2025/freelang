...
import { ParameterConstraintsEngine } from './parameter-constraints';
...
export class ConstraintSolverEngine {
  private genericsEngine: GenericsResolutionEngine;
  private parameterEngine: ParameterConstraintsEngine;

  constructor() {
    this.genericsEngine = new GenericsResolutionEngine();
      ...
      if (fn.body) {
        ...
        while ((match = extendsPattern.exec(fn.body)) !== null) {
          const constraint: TypeConstraint = {
            ...
            violated: false  // Assume satisfied for now
          };
          result.constraints.push(constraint);
          result.satisfied++;
        }
  ...
  /**
   * Type variable 판별 (대문자 시작 또는 단일 문자)
   */
    ...
    for (const fn of functions) {
      ...
      while ((match = traitBoundPattern.exec(fn.body)) !== null) {
        ...
        const constraint: TypeConstraint = {
          ...
          violated: false  // Assume satisfied (실제 구현에서는 trait 검증 필요)
        };
        result.constraints.push(constraint);
        result.satisfied++;
      }
